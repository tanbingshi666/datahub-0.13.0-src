...
public class EbeanAspectDao implements AspectDao, AspectMigrationsDao {
  ...
  public EbeanAspectDao(@Nonnull final Database server, EbeanConfiguration ebeanConfiguration) {
    ...
    if (ebeanConfiguration.getLocking().isEnabled()) {
      this.locks = CacheBuilder.newBuilder()
          ...
          .expireAfterWrite(ebeanConfiguration.getLocking().getDurationSeconds(), TimeUnit.SECONDS)
  ...
  @Override
  public long saveLatestAspect(@Nullable Transaction tx, @Nonnull final String urn, @Nonnull final String aspectName,
    ...
    if (oldAspectMetadata != null && oldTime != null) {
      ...
      saveAspect(tx, urn, aspectName, oldAspectMetadata, oldActor, oldImpersonator, oldTime, oldSystemMetadata,
          largestVersion, true);
    }

    // Save newValue as the latest version (v0)
    saveAspect(tx, urn, aspectName, newAspectMetadata, newActor, newImpersonator, newTime, newSystemMetadata,
  ...
  @Override
  public void saveAspect(@Nullable Transaction tx, @Nonnull final String urn, @Nonnull final String aspectName,
  ...
  public void saveAspect(@Nullable Transaction tx, @Nonnull final EntityAspect aspect, final boolean insert) {
    ...
    /**
     * save ingestion metadata 也即插入数据到 mysql 的 metadata_aspect_v2 表
     */
    saveEbeanAspect(tx, ebeanAspect, insert);
  }

  private void saveEbeanAspect(@Nullable Transaction tx, @Nonnull final EbeanAspectV2 ebeanAspect,
      final boolean insert) {
    validateConnection();
    if (insert) {
  ...
  public Map<String, Map<String, EntityAspect>> getLatestAspects(@Nonnull Map<String, Set<String>> urnAspects) {
    ...
    List<EbeanAspectV2.PrimaryKey> keys = urnAspects.entrySet()
        .stream()
  ...
  public long countEntities() {
    validateConnection();
    return _server.find(EbeanAspectV2.class).setDistinct(true).select(EbeanAspectV2.URN_COLUMN).findCount();
  }

  @Override
  public boolean checkIfAspectExists(@Nonnull String aspectName) {
    validateConnection();
    return _server.find(EbeanAspectV2.class).where().eq(EbeanAspectV2.ASPECT_COLUMN, aspectName).exists();
  }
  ...
  public Map<EntityAspectIdentifier, EntityAspect> batchGet(@Nonnull final Set<EntityAspectIdentifier> keys) {
    ...
    final Set<EbeanAspectV2.PrimaryKey> ebeanKeys =
        keys.stream().map(EbeanAspectV2.PrimaryKey::fromAspectIdentifier).collect(Collectors.toSet());
    ...
    return records.stream()
        .collect(Collectors.toMap(record -> record.getKey().toAspectIdentifier(), EbeanAspectV2::toEntityAspect));
  }
  ...
  @Override
  @Nonnull
  public <T> List<T> runInTransactionWithRetry(@Nonnull final Function<Transaction, T> block,
      @Nullable AspectsBatch batch, final int maxTransactionRetry) {

    LinkedList<T> result = new LinkedList<>();

    if (locks != null && batch != null) {
      Set<Urn> urnsWithKeyAspects = batch.getMCPItems()
          .stream()
      ...
      if (!urnsWithKeyAspects.isEmpty()) {
        ...
        if (!splitBatches.getSecond().getItems().isEmpty()) {
          result.add(runInTransactionWithRetryUnlocked(block, splitBatches.getSecond(), maxTransactionRetry));
        }

        // For each key aspect batch
        for (AspectsBatch splitBatch : splitBatches.getFirst()) {
          try {
            Lock lock = locks.get(splitBatch.getMCPItems().stream().findFirst().get().getUrn().toString());
            lock.lock();
            try {
  ...
  @Nonnull
  public <T> T runInTransactionWithRetryUnlocked(@Nonnull final Function<Transaction, T> block,
      @Nullable AspectsBatch batch, final int maxTransactionRetry) {
    ...
    do {
      try (Transaction transaction = _server.beginTransaction(
        ...
        if (exception instanceof DuplicateKeyException) {
          if (batch != null && batch.getItems()
              .stream()
              .allMatch(a -> a.getAspectName().equals(a.getEntitySpec().getKeyAspectSpec().getName()))) {
  ...
  @Override
  @Nonnull
  public List<EntityAspect> getAspectsInRange(@Nonnull Urn urn, Set<String> aspectNames, long startTimeMillis,
      long endTimeMillis) {
    validateConnection();
  ...
  private static String buildMetricName(EntitySpec entitySpec, AspectSpec aspectSpec, String status) {
    return String.join(MetricUtils.DELIMITER,
   ...
   */
  private static Pair<List<AspectsBatch>, AspectsBatch> splitByUrn(AspectsBatch batch, Set<Urn> urns,
      AspectRetriever aspectRetriever) {
    Map<Urn, List<MCPItem>> itemsByUrn = batch.getMCPItems().stream().collect(Collectors.groupingBy(MCPItem::getUrn));

    AspectsBatch other = AspectsBatchImpl.builder()
        .aspectRetriever(aspectRetriever)
        .items(itemsByUrn.entrySet()
            .stream()
    ...
    List<AspectsBatch> nonEmptyBatches = urns.stream()
        .map(urn -> AspectsBatchImpl.builder().aspectRetriever(aspectRetriever).items(itemsByUrn.get(urn)).build())
